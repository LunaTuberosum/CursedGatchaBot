...
async function eventGrabCheck(message, user, i, chance) {
    ...
    if (random == chance) {
        ...
        await message.channel.send(`${i.user} also got a <:brokenBrush:1376696067982098544>\`BROKEN PAINTBRUSH\`!`);
    }
}
...
async function pullMechanics(message, response, pokemonData1, pokemonData2) {
    ...
    const collector = response.createMessageComponentCollector({ componentType: ComponentType.Button, time: 25_000 });
